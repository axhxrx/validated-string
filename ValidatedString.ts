    ...
    const factory = {
        ...
        assert(value: string)
        {
            if (!validator(value))
            {
                ...
                throw new Error(`Invalid string did not pass validation: Supplied value "${supplied}" is not valid for validator "${name}"${description ? ` (${description})` : ''}.`);
            }
            return value as ValidatedString<ValidatorT>;
        },
    } as ValidatedStringFactory<ValidatorT>;

    const type = 'This value is fake! ValidatedString created it for evildoing, and you are holding it wrong!' as unknown as ValidatedString<ValidatorT>;
